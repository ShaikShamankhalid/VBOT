...
class SearchAnswerPipeline:
    def __init__(self, llm_model=None, tokenizer=None, embed_model=None) -> None:
        setup_env()
        config = ConfigurationManager()
        self.search_config = config.get_search_config()
        self.query_answer = SearchAndAnswer(
            self.search_config, llm_model, tokenizer, embed_model
        )
    ...
    def chainlit_prompt(self, query: str, prompt: str, hf_key, index, model):
        try:
            if index != "chat":
